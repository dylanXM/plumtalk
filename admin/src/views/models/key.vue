...
const rules = reactive<FormRules>({
  ...
  isDraw: [{ required: true, message: '请选择当前key是否作为系统绘画key', trigger: 'change' }],
  ...
  timeout: [{ required: true, message: '请填写超时时间 默认 60 单位（秒）', trigger: 'blur' }],
...
<template>
  <div>
    ...
    <page-main>
      <el-alert show-icon title="KEY池说明" description="所有key会根据你所选的模型自动分类、会以第一个加入的key为标准、建议所有同模型的key参数配置相同、否则可能特殊场景会有迷惑性。不同模型可以设置不同的扣费类型以及单次调用扣除的额度、额度不再以次为单位、改为(积分)概念！你可以设置模型最大上限和回复上限、以及最大轮次用以限制用户的最大上下文、当限制token后、用户端可选范围受其影响、轮次同理、到达轮次之后会移除掉更靠前的上下文！" type="success" />
    </page-main>
    <page-main style="width: 100%;">
      <el-table v-loading="loading" border :data="tableData" style="width: 100%;" size="large">
        ...
        <el-table-column prop="keyWeight" label="模型权重" width="90" align="center" />
        ...
        <el-table-column prop="keyStatus" align="center" label="key状态" width="110">
          <template #default="scope">
            <el-tag :type="scope.row.keyStatus === 1 ? 'success' : 'danger'">
              {{ scope.row.keyStatus === 1 ? '正常工作' : scope.row.keyStatus === -1 ? '已被封禁' : '余额耗尽 ' }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="maxResponseTokens" align="center" label="最大回复Token" width="160">
          <template #default="scope">
            <el-button type="info" text>
        ...
        <el-table-column prop="proxyUrl" align="center" label="变更提示信息" width="180">
          <template #default="scope">
            {{ scope.row.remark || '-' }}
          </template>
        </el-table-column>
      ...
      >
        ...
        <el-form-item v-if="[1, 4].includes(Number(formPackage.keyType))" label="是否支持绘画" prop="isDraw">
          ...
          >
            <template #content>
              <div style="width: 250px;">
                绘画来自于智谱AI的cogview-3模型、所以需要为官方的apiKey、请确定至少设置一张key为基础绘画key即可使用绘画功能！
              </div>
            </template>
            <el-icon class="ml-3 cursor-pointer">
              <QuestionFilled />
