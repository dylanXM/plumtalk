...
import { RECHARGE_TYPE_MAP, RECHARGE_TYPE_OPTIONS, USER_STATUS_MAP, USER_STATUS_TYPE_MAP } from '@/constants/index'
...
async function queryAllAccountLog() {
  try {
    loading.value = true
    const res = await ApiUserBalance.queryUserAccountLog(formInline)
  ...
  <div>
    <page-main>
      <el-form ref="formRef" :inline="true" :model="formInline">
        ...
        <el-form-item label="充值类型" prop="rechargeType">
          <el-select v-model="formInline.rechargeType" placeholder="请选择充值类型" clearable>
            <el-option v-for="item in RECHARGE_TYPE_OPTIONS" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <page-main style="width: 100%;">
      <el-table v-loading="loading" border :data="tableData" style="width: 100%;" size="large">
        ...
        <el-table-column prop="balanceInfo.useModel4Count" label="充值类型" width="160" align="center">
          <template #default="scope">
            <el-tag type="success">
              {{ scope.row?.rechargeType ? RECHARGE_TYPE_MAP[scope.row?.rechargeType] : '---' }}
            </el-tag>
          </template>
        </el-table-column>
