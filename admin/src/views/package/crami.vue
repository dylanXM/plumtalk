    ...
    <page-main>
      <el-form ref="formRef" :inline="true" :model="formInline">
        ...
        <el-form-item label="卡密状态" prop="status">
          <el-select v-model="formInline.status" placeholder="请选择卡密状态" clearable>
            <el-option v-for="item in CRAMI_STATUS_OPTIONS" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <div style="float: right;">
          <el-button v-if="selects.length" type="danger" @click="handleShowCrami">
            显示选中卡密
          </el-button>
          <el-button v-if="selects.length" type="danger" @click="batchDelCrami">
            批量删除卡密
          </el-button>
          <el-button v-if="selects.length" type="primary" @click="exportCrami">
            批量导出卡密
          </el-button>
          <el-button type="success" @click="openCreateCramiDialog">
            批量生成卡密
            <el-icon class="ml-3">
              <Plus />
            </el-icon>
...
<!--        <el-table-column prop="model4Count" label="高级模型额度" />-->
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-popconfirm title="确认删除此卡密么?" width="200" icon-color="red" @confirm="handleDeleteCrami(scope.row)">
              <template #reference>
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="!customCrami" label="套餐类型" prop="packageId">
          <el-select v-model.number="form.packageId" placeholder="请选择套餐类型" clearable style="width: 100%;">
