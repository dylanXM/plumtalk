...
import { NButton, NEmpty, NIcon, NInput, NInputGroup, NTabPane, NTabs, useMessage } from 'naive-ui'
...
<template>
  ...
  <div class="main min-h-screen bg-center dark:bg-[#2F2E34]" :class="[!darkMode ? 'lightBg' : 'darkBg', isMobile ? 'px-3' : 'px-10']">
    <TitleBar title="系统绘画" des="系统绘画、速度较快、同步等待到结束后在我的绘画中可以看到结果！" :padding="isMobile ? 2 : 20" />
      ...
      <NTabs type="line" animated class="mt-5" @update:value="updateTabs">
        <NTabPane name="all" tab="公共生成">
          <div v-if="allDrawList.length" class="min-h-screen">
            <GridManager use-propmpt :gap="8" pre-origin :data-list="allDrawList" :scale-width="50" @loadMore="loadMore" @usePropmptDraw="usePropmptDraw" />
          </div>
        ...
        <NTabPane name="mine" tab="我的生成">
          <div v-if="mineDrawList.length" class="min-h-screen">
            <GridManager use-propmpt :gap="8" pre-origin :data-list="mineDrawList" :scale-width="50" @loadMore="loadMore" @usePropmptDraw="usePropmptDraw" />
          </div>
          <NEmpty v-else size="huge" class="mt-20" description="暂无数据哟~" />
        </NTabPane>
      </NTabs>
